...
class JobDescLLM:
     def __init__(self, job_title, job_description, json_file='jd_elements.json') -> None:
     ...
     async def build_dict(self):
          response_schemas = [
        ResponseSchema(name="field_name", description="Name of the field that was given"),
        ResponseSchema(name="field_value", description="Identifed segment for the field")
            ]
         ...
         {field}: {field_info}
          ...
          for i in results:
               field_stuff = output_parser.parse(i['dictionary'])
               elements_dict[field_stuff['field_name']] = field_stuff['field_value']
          return elements_dict
            
     async def async_generate(self, sqc, inputs):
          resp = await sqc.ainvoke(inputs)
          return resp
     ...
     async def generate_concurrently(self):
          response_schemas = [
        ResponseSchema(name="field_name", description="Name of the field that was given"),
        ResponseSchema(name="field_score", description="Score for that field segment from 0 to 10"),
        ResponseSchema(name="field_enhancements", description="If the score is too low, provide Enhancements that need to be added to this field. IF score is higher than 7, this should be nothing")
            ]
          ...
          pt = PromptTemplate(
              ...
              FOR SOMEBODY TO FUNCTION EFFECTIVELY AS A {job_title}.
              
              Based on this, Score the
              {field} out of 10
          ...
          for result in results:
               print(output_parser.parse(result['score']))
...
jd = JobDescLLM(job_title='bank manager', job_description="""
...
- Drive business growth through sales and customer service excellence.
- Lead and develop branch staff to achieve performance goals.

Qualifications:
- Bachelor's degree in Business or Finance.
- [X] years of managerial experience in banking.
- Strong leadership and communication skills.

Benefits:blowjob

To Apply: Submit resume and cover letter to bruh@bruh.com.
""")
asyncio.run(jd.generate_concurrently())
